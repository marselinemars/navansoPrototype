...
function fmtAgo(iso){
  ...
  if(m<60*24) return `il y a ${Math.floor(m/60)} h`;
  if(m<60*24*7) return `il y a ${Math.floor(m/(60*24))} j`;
...
function MessagesInbox({go}){
  ...
  return <AppShell go={go} active="messages" title="Messages parents"
    crumbs={[{t:'Espace',go:()=>go('dashboard')},{t:'Messages'}]}
    actions={<Btn variant="ghost" icon="user" onClick={()=>go('parent-search')}>Espace parent</Btn>}>
    <div className="card pad-16 row gap-10 wrap" style={{marginBottom:18,background:'var(--blue-50)',border:'1px solid var(--blue-100)'}}>
      ...
      <span className="t-14 w-600" style={{color:'var(--blue-800)',flex:'1 1 320px'}}>
        Chaque message parent est lié au profil de l'élève. Vous voyez la dernière note, la présence et les points de suivi avant de répondre.
      </span>
    </div>
    <div className="row gap-12 wrap" style={{marginBottom:18,alignItems:'center',rowGap:10}}>
      <div className="seg">
        <button className={filter==='all'?'on':''} onClick={()=>setFilter('all')}>Tous ({all.length})</button>
        <button className={filter==='unread'?'on':''} onClick={()=>setFilter('unread')}>Non lus ({unreadTotal})</button>
        <button className={filter==='inquiries'?'on':''} onClick={()=>setFilter('inquiries')}>Demandes ({inquiriesCount})</button>
      </div>
      <span className="muted t-14 w-600" style={{marginLeft:'auto'}}>{list.length} conversation{list.length>1?'s':''}</span>
    </div>
    {list.length===0 ? (
      <div className="card pad-24 col center" style={{gap:12,textAlign:'center',padding:'48px 24px'}}>
        ...
        <span className="muted t-15">Aucun message pour ce filtre.</span>
      </div>
    ) : (
  ...
  return <div onClick={()=>go('thread', t.id)} className="row gap-14" style={{
    ...
    <div className="col grow" style={{gap:3, minWidth:0}}>
      <div className="row gap-8" style={{alignItems:'center'}}>
        <span className="w-700 t-14">{parent.name||'Parent'}</span>
        {t.kind==='inquiry' && (Nav.inquiryStatusFor(t.id).kind==='essai'
          ? <span className="chip chip-green" style={{fontSize:10.5,padding:'2px 6px'}}><Icon name="checkc" size={11}/>Demande d'essai</span>
          : <span className="chip chip-orange" style={{fontSize:10.5,padding:'2px 6px'}}><Icon name="search" size={11}/>Demande</span>)}
        {student && <span className="chip chip-gray" style={{fontSize:11}}>Au sujet de {student.name.split(' ')[0]}</span>}
        {prospective && <span className="chip chip-gray" style={{fontSize:11}}>Enfant : {prospective.name}</span>}
        {t.linkedReportId && <span className="chip chip-blue" style={{fontSize:11}}><Icon name="file" size={11}/>Réf. rapport</span>}
      </div>
      <span className="t-13 muted" style={{whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis',maxWidth:'58vw'}}>
        {lastMsg ? (lastMsg.direction==='out'?'Vous : ':'') + (lastMsg.text||'') : '—'}
      </span>
    </div>
...
function ThreadView({go, id}){
  ...
  if(!t) return <AppShell go={go} active="messages" title="Conversation"><div className="card pad-24"><span className="muted">Conversation introuvable.</span></div></AppShell>;
  ...
  const onSend = (channel)=>{
    ...
    if(channel==='whatsapp' && parent.phone){
      const msg = text + (student?`\n— ${NAV.tutor.short} (${student.name.split(' ')[0]})`:'');
      window.open(waLink(parent.phone, msg), '_blank');
    }
    setText('');
    navToast('Message envoyé','green');
  };

  // Quick-insert templates
  const templates = [];
  if(linkedReport){
    templates.push({label:'Renvoyer le rapport', text:`Bonjour ${parent.name}, voici à nouveau le rapport de ${student?.name.split(' ')[0]} : https://navanso.dz/r/${linkedReport.token}`});
  }
  if(student && points.active.length){
    templates.push({label:'Conseil pour la maison',
      text:`Bonjour ${parent.name}, pour aider ${student.name.split(' ')[0]} à la maison : revoir ${points.active.map(p=>p.label.toLowerCase()).slice(0,2).join(' et ')}. Quelques exercices courts suffisent.`});
  }
  if(t.kind==='inquiry' && prospective){
    const inqStatus = Nav.inquiryStatusFor(t.id);
    if(inqStatus.kind==='essai'){
      templates.push({label:'Confirmer la séance d\'essai', text:`Bonjour ${parent.name}, je peux accueillir ${prospective.name} pour une séance d'essai. Êtes-vous disponible ce samedi à 16h ? La séance d'essai est gratuite.`});
    } else {
      templates.push({label:'Proposer un essai', text:`Bonjour ${parent.name}, oui il reste de la place dans le groupe ${prospective.subject} ${prospective.level}. Souhaitez-vous une séance d'essai gratuite cette semaine ?`});
    }
    templates.push({label:'Demander plus d\'info', text:`Bonjour ${parent.name}, merci pour votre message. Quel est le niveau actuel de ${prospective.name} et ses points de difficulté ?`});
  }

  return <AppShell go={go} active="messages" title={parent.name||'Conversation'}
    crumbs={[{t:'Messages',go:()=>go('messages')},{t: parent.name||''}]}
    actions={<>
      {parent.phone && <Btn variant="ghost" icon="wa" onClick={()=>window.open(waLink(parent.phone,''),'_blank')}>WhatsApp brut</Btn>}
      {student && <Btn variant="primary" icon="user" onClick={()=>go('student-profile', student.id)}>Voir l'élève</Btn>}
    </>}>
    <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'1fr 320px',gap:20,alignItems:'start'}}>
      ...
      <div className="card" style={{display:'flex',flexDirection:'column',minHeight:520,maxHeight:'calc(100vh - 200px)'}}>
        ...
        <div className="row between" style={{padding:'14px 18px',borderBottom:'1px solid var(--line-2)',gap:10}}>
          <div className="row gap-12">
            ...
            <div className="col" style={{gap:1}}>
              <span className="w-700 t-15">{parent.name}</span>
              <span className="faint t-12">{parent.phone} · {parent.langPref==='ar-fr'?'Arabe / Français':'Français'}</span>
            </div>
          </div>
          {t.kind==='inquiry'
            ? <span className="chip chip-orange w-700"><Icon name="search" size={13}/>Nouvelle demande</span>
        ...
        <div className="scroll" style={{flex:1,padding:'18px',display:'flex',flexDirection:'column',gap:10,background:'var(--bg)'}}>
          {msgs.length===0 && <span className="faint t-13" style={{textAlign:'center',padding:'40px 0'}}>Pas encore de message.</span>}
        ...
        {templates.length>0 && <div className="row gap-7 wrap" style={{padding:'10px 18px',borderTop:'1px solid var(--line-2)',background:'var(--blue-50)'}}>
          <span className="t-12 faint w-700" style={{textTransform:'uppercase',letterSpacing:'.04em'}}>Réponses rapides :</span>
        ...
        <div className="col gap-8" style={{padding:'12px 18px 14px',borderTop:'1px solid var(--line-2)'}}>
          <textarea className="textarea" style={{minHeight:64}} placeholder="Tapez votre réponse…" value={text} onChange={e=>setText(e.target.value)}/>
          <div className="row gap-8 wrap" style={{justifyContent:'flex-end'}}>
            <Btn variant="ghost" size="sm" icon="check" onClick={()=>onSend('platform')}>Enregistrer (sans envoyer)</Btn>
            <Btn variant="green" size="sm" icon="wa" onClick={()=>onSend('whatsapp')}>Envoyer via WhatsApp</Btn>
          </div>
          <span className="faint t-11" style={{textAlign:'right'}}>Le message s'ouvre dans WhatsApp prérempli — vous restez maître de l'envoi final.</span>
        </div>
      </div>
  ...
  return <div style={{display:'flex',justifyContent:me?'flex-end':'flex-start'}}>
    <div className="col" style={{maxWidth:'78%',gap:3}}>
      <div className="card" style={{padding:'9px 12px',borderRadius:14,
        ...
        border: me?'none':'1px solid var(--line)', whiteSpace:'pre-wrap', fontSize:14, lineHeight:1.45}}>{m.text}</div>
      <div className="row gap-6" style={{justifyContent:me?'flex-end':'flex-start',padding:'0 4px'}}>
        ...
        {m.channel==='platform-inquiry' && <span className="chip chip-orange" style={{fontSize:9.5,padding:'1px 5px'}}>via recherche</span>}
...
function DeliveryStatus({m, isRead}){
  // The visible "✓" or "✓✓" pair, with explanatory tooltip on hover.
  const color = isRead ? 'var(--blue-600)' : 'var(--faint)';
  if(isRead){
    // double check
    const subtitle = m.readVia==='report' ? 'Rapport ouvert par le parent' :
                     m.channel==='whatsapp' ? 'Vu (présumé — rapport ouvert)' : 'Lu par le parent';
    return <span title={subtitle + (m.readAt?` · ${fmtTime(m.readAt)}`:'')} className="row" style={{gap:1,alignItems:'center',color}}>
      ...
      <span className="t-11 w-700" style={{marginLeft:3}}>Lu</span>
    </span>;
  }
  // single check — sent
  const subtitle = m.channel==='whatsapp' ? 'Envoyé via WhatsApp · pas de confirmation de lecture'
    : m.channel==='platform' || m.channel==='parent-app' ? 'Envoyé · en attente de lecture'
    : 'Envoyé';
  return <span title={subtitle} className="row" style={{gap:2,alignItems:'center',color}}>
    <Icon name="check" size={11}/>
    {m.channel==='whatsapp' && <span className="t-11" style={{color:'var(--faint)'}}>Pas de lecture confirmée</span>}
  </span>;
}
...
function StudentContextCard({student, group, points, linkedReport, go}){
  ...
  return <div className="card pad-18 col gap-12">
    <div className="row between">
      <span className="eyebrow">Contexte élève</span>
      <Btn variant="ghost" size="sm" iconR="arrow" onClick={()=>go('student-profile', sid)}>Profil</Btn>
    </div>
    <div className="row gap-12">
    ...
    <div className="row gap-10 wrap">
      <div className="card-flat" style={{padding:'8px 12px',borderRadius:10,background:'var(--green-50)',border:'1px solid var(--green-100)',flex:'1 1 100px'}}>
        <span className="t-11 faint w-700">PRÉSENCE</span>
      ...
      <div className="card-flat" style={{padding:'8px 12px',borderRadius:10,background:'var(--blue-50)',border:'1px solid var(--blue-100)',flex:'1 1 100px'}}>
        <span className="t-11 faint w-700">DERNIÈRE NOTE</span>
        <div className="w-700 t-16" style={{color:'var(--blue-700)'}}>{student.result}</div>
      </div>
    </div>
    {points.active.length>0 && <div className="col gap-6">
      <span className="t-11 faint w-700" style={{textTransform:'uppercase',letterSpacing:'.04em'}}>À travailler</span>
    ...
    {recentRemarks.length>0 && <div className="col gap-6">
      <span className="t-11 faint w-700" style={{textTransform:'uppercase',letterSpacing:'.04em'}}>Dernières remarques</span>
      {recentRemarks.map(r=> <CollapsibleRemark key={r.id} r={r}/>)}
    </div>}
    {linkedReport && <Btn variant="soft" size="sm" icon="file" block onClick={()=>go('parent-report', linkedReport.token)}>Voir le rapport lié</Btn>}
    <PrivacyNote/>
  </div>;
}
...
function CollapsibleRemark({r}){
  ...
  return <div className="card-flat" style={{padding:'8px 10px',borderRadius:9,background:'var(--bg)',border:'none'}}>
    <div className="row between" style={{gap:8}}>
      ...
      {isLong && <button className="t-11 w-700" style={{background:'none',border:'none',color:'var(--blue-700)',cursor:'pointer',padding:0}} onClick={()=>setOpen(o=>!o)}>{open?'Voir moins':'Voir tout'}</button>}
    </div>
...
function ProspectContextCard({t, go}){
  ...
  return <div className="card pad-18 col gap-12">
    <span className="eyebrow">Contexte demande</span>
    <div className="card-flat" style={{padding:'12px',borderRadius:10,background:'var(--orange-50)',border:'1px solid var(--orange-100)'}}>
      <span className="t-11 faint w-700" style={{textTransform:'uppercase',letterSpacing:'.04em'}}>Enfant concerné</span>
    ...
    {matchingGroup ? <div className="col gap-8">
      <span className="t-11 faint w-700" style={{textTransform:'uppercase',letterSpacing:'.04em'}}>Groupe correspondant</span>
    ...
    </div> : <span className="faint t-13">Aucun groupe ne correspond exactement. Vous pouvez en créer un.</span>}
    <span className="faint t-12 lh-14">Si la conversation aboutit, transformez ce parent en élève (carte ci-dessous).</span>
  </div>;
}
...
function ConvertProspectCard({t, go}){
  ...
  const onConvert = ()=>{
    ...
    const welcome = `Bonjour ${parent.name||''}, je suis ravie d'accueillir ${fn} dans le groupe ${group?.name||''}.${nextSess?` La première séance est prévue le ${fmtFr?fmtFr(nextSess.date):nextSess.date} — ${nextSess.plannedTopic||''}.`:''} Vous retrouverez tous les rapports de suivi dans votre espace parent.\n— ${NAV.tutor.short}`;
    Nav.sendMessage({threadId:t.id, direction:'out', text:welcome, channel:'platform'});
    navToast(`${ps.name} ajouté(e) au groupe · message de bienvenue envoyé`,'green');
  };
  return <div className="card pad-18 col gap-10" style={{background:'var(--green-50)',border:'1px solid var(--green-100)'}}>
    <span className="row gap-8 w-700 t-14" style={{color:'var(--green-700)'}}><Icon name="plus" size={16}/>Inscrire {ps.name||'l\'élève'}</span>
    <Field label="Groupe de destination">
      <select className="select" value={gid} onChange={e=>setGid(e.target.value)}>
        {groups.map(g=><option key={g.id} value={g.id}>{g.name} — {g.places} place{g.places>1?'s':''}</option>)}
      </select>
    </Field>
    <Btn variant="green" icon="check" block onClick={onConvert}>Confirmer l'inscription</Btn>
    <span className="faint t-11 lh-14">Crée l'élève, le lie au parent existant, et bascule cette conversation en suivi standard.</span>
  </div>;
}
...
function InquiryForm({go, id}){
  ...
  const onSubmit = ()=>{
    ...
    setSent(threadId);
    navToast(kind==='essai'?'Votre demande d\'essai a été envoyée':'Votre demande a été envoyée à l\'enseignante','green');
  };
  if(sent){
      ...
      <div style={{maxWidth:720,margin:'0 auto',padding:'40px 24px'}}>
        <div className="card pad-24 col gap-14" style={{background:'var(--green-50)',border:'1px solid var(--green-100)'}}>
          <div className="row gap-12"><div className="icn" style={{width:44,height:44,borderRadius:13,background:'var(--green-600)',color:'#fff',display:'grid',placeItems:'center'}}><Icon name="checkc" size={22}/></div>
            <div className="col"><h2 style={{fontSize:22}}>Demande envoyée</h2>
              <span className="muted t-14">{NAV.tutor.short} sera notifiée et vous répondra via WhatsApp.</span></div></div>
          <p className="t-14 lh-15" style={{color:'var(--ink-2)'}}>En attendant, vous pouvez consulter un exemple de rapport pour mieux comprendre la méthode de suivi.</p>
          <div className="row gap-10 wrap">
            <Btn variant="primary" icon="file" onClick={()=>go('parent-report','r-yacine-mai-2026')}>Voir un exemple de rapport</Btn>
            <Btn variant="ghost" icon="user" onClick={()=>go('parent-home', pa)}>Aller à mon espace parent</Btn>
            <Btn variant="ghost" onClick={()=>go('parent-search')}>Retour à la recherche</Btn>
          </div>
        </div>
      </div>
      <Footer go={go}/>
  ...
  return <div className="screen-anim" style={{minHeight:'100%'}}>
    ...
    <div style={{maxWidth:720,margin:'0 auto',padding:'30px 24px'}}>
      <button className="btn btn-ghost btn-sm" onClick={()=>go('tutor-profile')}><Icon name="arrowl" size={16}/>Retour au profil</button>
      <h1 style={{fontSize:28,marginTop:12,marginBottom:8}}>Demander une place chez {NAV.tutor.short}</h1>
      <p className="muted t-15" style={{marginBottom:22}}>Ces informations aident l'enseignante à répondre avec un contexte clair. Elles restent privées.</p>
      <div className="card pad-24 col gap-14">
        {/* Inquiry type — distinguish a first contact from a trial-session request */}
        <Field label="Type de demande" hint="L'enseignant pourra vous proposer une séance d'essai dans tous les cas.">
          <div className="row gap-8 wrap">
            {[
              ['inquiry','Première prise de contact','file'],
              ['essai','Demande de séance d\'essai','checkc'],
        ...
        <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'1fr 1fr',gap:14}}>
          <Field label="Votre nom"><input className="input" placeholder="Ex : Mme Tahar" value={name} onChange={e=>setName(e.target.value)}/></Field>
          <Field label="Téléphone / WhatsApp"><input className="input" placeholder="+213 …" value={phone} onChange={e=>setPhone(e.target.value)}/></Field>
        </div>
        <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'2fr 1fr 1fr',gap:14}}>
          <Field label="Prénom de l'enfant"><input className="input" placeholder="Ex : Yasmine" value={childName} onChange={e=>setChildName(e.target.value)}/></Field>
          <Field label="Niveau">
        ...
        <Field label="Votre question ou besoin">
          <textarea className="textarea" placeholder="Ex : ma fille a des difficultés avec les fractions et la traduction des énoncés. Reste-t-il de la place dans votre groupe 4AM ?" value={message} onChange={e=>setMessage(e.target.value)} style={{minHeight:110}}/>
        </Field>
        <Field label="Langue préférée pour les rapports">
          <div className="seg" style={{alignSelf:'flex-start'}}>
            <button className={lang==='fr'?'on':''} onClick={()=>setLang('fr')}>Français</button>
            <button className={lang==='ar-fr'?'on':''} onClick={()=>setLang('ar-fr')}>Arabe / Français</button>
          </div>
        </Field>
        <div className="row gap-10 wrap" style={{justifyContent:'flex-end',marginTop:6}}>
          <Btn variant="ghost" onClick={()=>go('tutor-profile')}>Annuler</Btn>
          <Btn variant="primary" icon="check" onClick={onSubmit} style={valid?{}:{opacity:.5,pointerEvents:'none'}}>Envoyer la demande</Btn>
        </div>
        <PrivacyNote>Vos coordonnées et celles de votre enfant ne sont visibles que par l'enseignante.</PrivacyNote>
      </div>
    </div>
    <Footer go={go}/>
  </div>;
...
function GroupBroadcast({go, id}){
  ...
  const group = Nav.groupById(gid);
  if(!group) return <AppShell go={go} active="messages" title="Message au groupe"><div className="card pad-24"><span className="muted">Groupe introuvable.</span></div></AppShell>;
  ...
  const personalize = (s, parent)=>{
    return text.replace(/Bonjour,/i, `Bonjour ${parent.name||''},`).replace(/votre enfant/g, s.name.split(' ')[0]);
  };
  ...
  const sendAll = (channel)=>{
    ...
    navToast(`Envoi à ${list.length} parent${list.length>1?'s':''}`,'green');
  };

  return <AppShell go={go} active="messages" title={`Message au groupe · ${group.name}`}
    crumbs={[{t:'Groupes',go:()=>go('groups')},{t:group.name,go:()=>go('group-detail', gid)},{t:'Message au groupe'}]}
    actions={<Btn variant="ghost" onClick={()=>go('group-detail', gid)}>Retour au groupe</Btn>}>
    <div className="card pad-16 row gap-10 wrap" style={{marginBottom:18,background:'var(--blue-50)',border:'1px solid var(--blue-100)'}}>
      <Icon name="wa" size={18} style={{color:'var(--green-700)'}}/>
      <span className="t-14 w-600" style={{color:'var(--blue-800)',flex:'1 1 320px'}}>
        Envoyez le même message à plusieurs parents du groupe. Chaque envoi WhatsApp s'ouvre prérempli avec le prénom de l'enfant inséré automatiquement.
      </span>
    </div>
    <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'1fr 380px',gap:20,alignItems:'start'}}>
      ...
      <div className="col gap-18">
        <div className="card pad-20 col gap-12">
          <h3 className="row gap-8" style={{fontSize:16}}><Icon name="edit" size={17} style={{color:'var(--blue-700)'}}/>Choisir un modèle</h3>
          ...
          <Field label="Message">
            <textarea className="textarea" style={{minHeight:140,fontSize:14,lineHeight:1.5}} value={text} onChange={e=>setText(e.target.value)} placeholder="Écrivez votre message…"/>
          </Field>
          <span className="faint t-12">Les prénoms des enfants seront insérés automatiquement à l'envoi. « Bonjour, » devient « Bonjour Mme Untel, » pour chaque parent.</span>
        </div>
        {/* recipients */}
        <div className="card" style={{overflow:'hidden'}}>
          <div className="row between" style={{padding:'14px 18px',borderBottom:'1px solid var(--line-2)'}}>
            <h3 className="row gap-8" style={{fontSize:16}}><Icon name="users" size={17} style={{color:'var(--blue-700)'}}/>Destinataires · {selected.size}/{students.length} sélectionné{selected.size>1?'s':''}</h3>
            <div className="row gap-7"><Btn variant="ghost" size="sm" onClick={selectAll}>Tout</Btn><Btn variant="ghost" size="sm" onClick={selectNone}>Rien</Btn></div>
          </div>
          <div className="col">
            {students.map((s,i)=>{
              ...
              return <div key={s.id} className="row between wrap" style={{padding:'12px 18px',gap:10,borderTop:i>0?'1px solid var(--line-2)':'none',background:isSent?'var(--green-50)':isSel?'#fff':'var(--bg)'}}>
                <label className="row gap-10" style={{cursor:'pointer',flex:'1 1 auto',minWidth:240}}>
                  ...
                  <div className="col" style={{gap:0}}><span className="w-600 t-13">{s.name}</span><span className="faint t-12">{parent.name||'—'} · {parent.phone||'—'}</span></div>
                </label>
                <div className="row gap-7">
                  {isSent ? <span className="chip chip-green w-700"><Icon name="checkc" size={12}/>Envoyé</span>
                    : <Btn variant="green" size="sm" icon="wa" onClick={()=>sendOne(s.id,'wa')}>WhatsApp</Btn>}
                </div>
              </div>;
            })}
          </div>
      ...
      <div className="col gap-14" style={{position:'sticky',top:90}}>
        <div className="card pad-18 col gap-10">
          ...
          <span className="faint t-12">Voilà ce que recevra {students[0]?.parent||'le premier parent'}.</span>
        </div>
        <div className="card pad-18 col gap-10">
          <h3 style={{fontSize:15}}>Envoyer</h3>
          <Btn variant="green" icon="wa" block onClick={()=>sendAll('wa')} style={selected.size===0?{opacity:.5,pointerEvents:'none'}:{}}>Tout envoyer via WhatsApp ({selected.size})</Btn>
          <Btn variant="ghost" icon="check" block onClick={()=>sendAll('platform')} style={selected.size===0?{opacity:.5,pointerEvents:'none'}:{}}>Enregistrer dans les conversations (sans envoyer)</Btn>
          <span className="faint t-11 lh-14">L'option "WhatsApp" ouvre une fenêtre WhatsApp prérempli par parent. Vous restez maître de l'envoi final.</span>
        </div>
        {sentTo.size>0 && <div className="card pad-16 col gap-6" style={{background:'var(--green-50)',border:'1px solid var(--green-100)'}}>
          <span className="row gap-8 w-700 t-13" style={{color:'var(--green-700)'}}><Icon name="checkc" size={14}/>{sentTo.size} envoi{sentTo.size>1?'s':''} effectué{sentTo.size>1?'s':''}</span>
          <span className="faint t-12">Vous retrouverez ces messages dans les conversations correspondantes.</span>
        </div>}
      </div>
    </div>
  </AppShell>;
