...
const TODO_TAGS = {
  'lesson-prep':       {label:'Préparation', icon:'book',     color:'blue'},
  'parent-followup':   {label:'Parent',      icon:'wa',       color:'green'},
  'student-followup':  {label:'Élève',       icon:'user',     color:'orange'},
  'admin':             {label:'Admin',       icon:'settings', color:'gray'},
  'general':           {label:'Général',     icon:'clipboard',color:'gray'},
};
...
function TodoRow({t, compact}){
  ...
  return <div className="row gap-12" style={{padding:compact?'10px 12px':'14px 16px',borderRadius:10,background:t.done?'var(--bg)':'#fff',border:'1px solid var(--line-2)',opacity:t.done?.6:1,alignItems:'flex-start'}}>
    ...
    <div className="col grow" style={{gap:6,minWidth:0}}>
      <span className={`w-600 t-${compact?'13':'14'}`} style={{textDecoration:t.done?'line-through':'none',color:'var(--ink)',lineHeight:1.35}}>{t.text}</span>
      <div className="row gap-8 wrap" style={{alignItems:'center',rowGap:6}}>
        ...
        {prio==='high' && <span className="chip chip-weak" style={{fontSize:10.5,padding:'3px 7px'}}>⚡ Priorité</span>}
        {group && <span className="faint t-11">· {group.name}</span>}
        {student && <span className="faint t-11">· {student.name}</span>}
        {session && <span className="faint t-11">· séance {fmtFr?fmtFr(session.date):session.date}</span>}
      </div>
    </div>
...
function TodoComposer({defaultTag, groupId, studentId, sessionId, placeholder, onAdded}){
  ...
  const submit = ()=>{ if(!text.trim()) return;
    ...
    setText(''); navToast('Tâche ajoutée','green'); onAdded&&onAdded();
  };
  return <div className="col gap-10">
    <div className="row gap-10 wrap" style={{alignItems:'stretch',rowGap:10}}>
      <input className="input" style={{flex:'1 1 240px'}} placeholder={placeholder||'Ajouter une tâche…'} value={text} onChange={e=>setText(e.target.value)} onKeyDown={e=>{if(e.key==='Enter')submit();}}/>
      <Btn variant="primary" size="sm" icon="plus" onClick={submit}>Ajouter</Btn>
    </div>
    <div className="row gap-7 wrap" style={{rowGap:8,alignItems:'center'}}>
      ...
      <span style={{flex:1}}/>
      <button className={`pick ${prio==='high'?'on':''}`} onClick={()=>setPrio(prio==='high'?'normal':'high')} style={{fontSize:11.5,padding:'5px 10px',color: prio==='high'?'#fff':'var(--muted)', background: prio==='high'?'var(--alert)':'transparent', borderColor: prio==='high'?'var(--alert)':'var(--line)'}}>⚡ Priorité</button>
    </div>
  </div>;
}

/* ----- a tiny card variant used as an embed on Group / Student / SessionEntry ----- */
function ScopedTodos({groupId, studentId, sessionId, title='Mes tâches sur cette page', defaultTag, emptyHint}){
  useStore();
  ...
  return <div className="card pad-20 col gap-14">
    <div className="row between wrap" style={{gap:8}}>
      <h3 className="row gap-8" style={{fontSize:15}}><Icon name="target" size={16} style={{color:'var(--blue-700)'}}/>{title}</h3>
      <span className="faint t-12 w-600">{list.length} en cours</span>
    </div>
    <TodoComposer defaultTag={defaultTag} groupId={groupId} studentId={studentId} sessionId={sessionId}
      placeholder={studentId?'Ex : préparer une fiche pour cet élève…':sessionId?'Ex : revoir l\'exemple des signes…':'Ex : préparer le prochain chapitre…'}/>
    <div className="col gap-8">
      {list.length===0 && <span className="faint t-13">{emptyHint||'Aucune tâche pour l\'instant.'}</span>}
...
function TodosList({go}){
  ...
  return <AppShell go={go} active="todos" title="Mes tâches"
    crumbs={[{t:'Espace',go:()=>go('dashboard')},{t:'Mes tâches'}]}
    actions={null}>
    <div className="card pad-16 row gap-10 wrap" style={{marginBottom:18,background:'var(--blue-50)',border:'1px solid var(--blue-100)'}}>
      ...
      <span className="t-14 w-600" style={{color:'var(--blue-800)',flex:'1 1 320px'}}>
        Vos tâches personnelles — préparation de cours, suivis parents, choses à ne pas oublier. Vous pouvez aussi ajouter une tâche directement depuis un groupe, un élève ou une séance.
      </span>
    </div>
    <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'1fr 320px',gap:20,alignItems:'start'}}>
      <div className="col gap-14">
        ...
        <div className="card pad-18 col gap-10">
          <h3 className="row gap-8" style={{fontSize:16}}><Icon name="plus" size={17} style={{color:'var(--blue-700)'}}/>Nouvelle tâche</h3>
          <TodoComposer placeholder="Ex : préparer une fiche pour le BEM blanc…"/>
        </div>
        {/* tabs */}
        <div className="row gap-12" style={{borderBottom:'1px solid var(--line-2)',marginTop:4,marginBottom:6}}>
          {[['active', `En cours (${counts.all})`], ['done', `Terminées (${counts.done})`]].map(([k,lab])=>
        ...
        <div className="row gap-8 wrap" style={{rowGap:10}}>
          {[['all','Toutes', counts.all], ...Object.entries(TODO_TAGS).map(([k,m])=>[k,m.label,counts[k]])].map(([k,lab,n])=>
        ...
        <div className="row gap-8 wrap" style={{rowGap:10,alignItems:'center'}}>
          <span className="faint t-12 w-700" style={{marginRight:2}}>Portée :</span>
          {[['all','Toutes'],['unscoped','Sans portée'],['group','Liées à un groupe'],['student','Liées à un élève'],['session','Liées à une séance']].map(([k,lab])=>
            <button key={k} className={`pick ${filterScope===k?'on':''}`} onClick={()=>setFilterScope(k)} style={{fontSize:11.5,padding:'4px 10px'}}>{lab}</button>
          )}
        </div>
        {/* list */}
        <div className="col gap-8">
          {list.length===0 && <div className="card pad-24 col center" style={{gap:8,textAlign:'center'}}><Icon name="check" size={26} style={{color:'var(--green-600)'}}/><span className="muted">Aucune tâche dans cette catégorie.</span></div>}
      ...
      <div className="col gap-14" style={{position:'sticky',top:90}}>
        ...
        <div className="card pad-20 col gap-14">
          <h3 style={{fontSize:15}}>Suivis parents en attente</h3>
          <div className="col gap-12">
            {(()=>{const list=Nav.todosActive().filter(t=>t.tag==='parent-followup'); return list.length===0?<span className="faint t-13">Aucun.</span>:list.map(t=><div key={t.id} className="col" style={{gap:3}}>
