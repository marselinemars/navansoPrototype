...
function Planning({go}){
  ...
  return <AppShell go={go} active="planning" title={isAr?'جدول الأسبوع':'Planning de la semaine'}
    crumbs={[{t:isAr?'الفضاء':'Espace',go:()=>go('dashboard')},{t:isAr?'الجدول':'Planning'}]}
    actions={<Btn variant="primary" icon="plus" onClick={()=>go('session-plan','new/'+(Nav.groupsAll()[0]||{}).id)}>{isAr?'حصّة جديدة':'Nouvelle séance'}</Btn>}>
    {next && <div className="card pad-18 row between wrap" style={{marginBottom:20,background:'var(--green-50)',border:'1px solid var(--green-100)',gap:14}}>
      <div className="row gap-12">
        ...
        <div className="col" style={{gap:2}}>
          <span className="w-700 t-15">{isAr?'الحصّة القادمة':'Prochaine séance'} · {fmtFr(next.date)}</span>
          <span className="muted t-13">{groups.find(g=>g.id===next.groupId)?.name} — {next.plannedTopic}</span>
        </div>
      </div>
      <Btn variant="green" icon="arrow" onClick={()=>go('session-entry', next.id)}>{isAr?'تحضير / إدخال':'Préparer / saisir'}</Btn>
    </div>}
    <div className="card" style={{overflow:'hidden'}}>
      <div className="row between wrap" style={{padding:'12px 18px',borderBottom:'1px solid var(--line-2)',gap:10}}>
        <div className="row gap-8">
          <button className="btn btn-ghost btn-sm btn-icon" onClick={()=>setWeekOffset(w=>w-1)} title={isAr?'الأسبوع السّابق':'Semaine précédente'}><Icon name="arrowl" size={15}/></button>
          <button className="btn btn-ghost btn-sm" onClick={()=>setWeekOffset(0)} style={weekOffset===0?{background:'var(--blue-50)',color:'var(--blue-700)',fontWeight:700}:{}}>{isAr?'اليوم':'Aujourd\'hui'}</button>
          <button className="btn btn-ghost btn-sm btn-icon" onClick={()=>setWeekOffset(w=>w+1)} title={isAr?'الأسبوع القادم':'Semaine suivante'}><Icon name="arrow" size={15}/></button>
          <span className="w-700 t-14" style={{marginLeft:8}}>{isAr?'أسبوع':'Semaine du'} {fmtFr(sunday.toISOString())}</span>
          {weekOffset!==0 && <span className="chip chip-gray" style={{fontSize:11}}>{weekOffset>0?'+':''}{weekOffset} {isAr?'أسبوع':'sem.'}</span>}
        </div>
        ...
        {days.map((d,i)=>{
          ...
          return <div key={i} style={{minHeight:170,borderLeft:i>0?'1px solid var(--line-2)':'none',borderTop:'1px solid var(--line-2)',padding:10,background:isWeekend?'var(--bg)':'#fff'}}>
            <div className="col" style={{gap:1,marginBottom:8}}>
              <span className="t-11 faint w-700" style={{textTransform:'uppercase',letterSpacing:'.04em'}}>{labels[d.getDay()]}{isWeekend && <span style={{marginLeft:4,opacity:.7}}>· {isAr?'عطلة':'weekend'}</span>}</span>
            ...
            <div className="col gap-6">
              {list.map(s=>{
                ...
                return <div key={s.id} onClick={()=>go('session-entry', s.id)} className="card-flat" style={{cursor:'pointer',padding:'7px 9px',borderRadius:8,background: tone==='orange'?'var(--orange-50)':'var(--green-50)',border:'1px solid '+(tone==='orange'?'var(--orange-100)':'var(--green-100)')}}>
                  ...
                  <div className="t-12 lh-13" style={{color:'var(--ink-2)'}}>{s.plannedTopic?(txData?txData(s.plannedTopic):s.plannedTopic):'—'}</div>
                </div>;
              })}
    ...
    <div className="card pad-20 col gap-14" style={{marginTop:18}}>
      <h3 style={{fontSize:16}}>{isAr?'ملخّص الأفواج':'Récap des groupes'}</h3>
      <div className="col">
