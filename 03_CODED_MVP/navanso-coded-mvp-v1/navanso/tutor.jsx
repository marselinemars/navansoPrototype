...
function AppShell({go, active, title, crumbs, actions, children, maxw=1180}){
  ...
  const nav=[
    [isAr?'الفضاء':'Espace',null,[
      ['dash',isAr?'لوحة التّحكّم':'Tableau de bord','dashboard'],
      ['users',isAr?'أفواجي':'Mes groupes','groups'],
      ['user',isAr?'تلاميذي':'Mes élèves','students'],
      ['wa',isAr?'الرّسائل':'Messages','messages', unread||null],
      ['clipboard',isAr?'التّقييمات':'Évaluations','assessment'],
      ['file',isAr?'التّقارير':'Rapports','reports', reportsPending||null],
    ]],
    [isAr?'أدوات':'Outils',null,[
      ['calendar',isAr?'الجدول':'Planning','planning'],
      ['target',isAr?'مهامي':'Mes tâches','todos', todosCount||null],
      ['chart',isAr?'المدفوعات':'Paiements','payments', overdue||null],
    ]],
  ];
  const map={
  ...
  return <div className="shell">
    ...
    <aside className={`side ${isMobile && sideOpen ? 'open' : ''}`}>
      {isMobile && <div className="side-close">
        <button onClick={()=>setSideOpen(false)} title={isAr?'إغلاق':'Fermer'}><Icon name="x" size={18}/></button>
      </div>}
      ...
      <div style={{marginTop:'auto'}}>
        <div className="nav-i" onClick={()=>go('tutor-onboarding')}><Icon name="settings"/>{isAr?'ملفّي':'Mon profil'}</div>
        <div className="nav-i" onClick={()=>go('tutor-profile')}><Icon name="eye"/>{isAr?'عرض الملف العمومي':'Voir profil public'}</div>
        <div className="card-flat" style={{marginTop:10,padding:12,borderRadius:12,background:'var(--blue-50)',border:'none'}}>
          <div className="row gap-10">
            ...
            <div className="col grow" style={{gap:0}}><span className="w-700 t-13">{isAr?'الأستاذة أمينة':'Mme Amina'}</span><span className="faint t-12">{isAr?'خطة الأستاذ':'Plan Enseignant'}</span></div>
            <button title={isAr?'تسجيل الخروج':'Se déconnecter'} className="btn btn-ghost btn-sm btn-icon" onClick={()=>{ NavAuth.logout(); navToast(isAr?'تمّ تسجيل خروجك':'Vous êtes déconnectée','blue'); go('landing'); }}><Icon name="arrow" size={14} style={{transform:'rotate(180deg)'}}/></button>
          </div>
        </div>
      </div>
    </aside>
    ...
    <div className="main">
      <div className="topbar" style={{position:'relative'}}>
        {isMobile && <button className="btn btn-ghost btn-icon" onClick={()=>setSideOpen(true)} title={isAr?'القائمة':'Menu'} style={{flex:'none',alignSelf:'flex-start'}}><Icon name="menu" size={18}/></button>}
        ...
        <div className="row gap-10" style={{marginLeft:'auto',position:'relative'}}>
          <div className="input-icon" style={{width:220,position:'relative'}}>
            <Icon name="search"/>
            <input className="input" style={{padding:'9px 12px 9px 38px',fontSize:13.5}} placeholder={isAr?'البحث عن تلميذ…':'Rechercher un élève…'} value={studentSearch} onChange={e=>setStudentSearch(e.target.value)}/>
            {studentSearchOpen && <div className="card" style={{position:'absolute',top:'calc(100% + 6px)',right:0,left:0,boxShadow:'var(--sh-3)',padding:6,zIndex:50,maxHeight:260,overflowY:'auto'}}>
              {matches.length===0 ? <div className="t-13 muted" style={{padding:'10px 12px'}}>{isAr?'لا يوجد تلميذ':'Aucun élève'}</div>
              : matches.map(s=><button key={s.id} onClick={()=>{setStudentSearch('');go('student-profile',s.id);}} style={{display:'flex',width:'100%',gap:10,padding:'8px 10px',border:'none',background:'transparent',cursor:'pointer',borderRadius:8,alignItems:'center'}}>
                ...
                <div className="col" style={{gap:0,textAlign:'left'}}><span className="w-600 t-13">{s.name}</span><span className="faint t-11">{isAr?txData(s.parent):s.parent}</span></div>
              </button>)}
            </div>}
          </div>
  ...
  return <>
    ...
    <div className="card" style={{position:'absolute',top:'calc(100% + 6px)',right:90,width:380,boxShadow:'var(--sh-4)',padding:0,zIndex:50,maxHeight:520,overflow:'hidden',display:'flex',flexDirection:'column'}}>
      <div className="row between" style={{padding:'12px 14px',borderBottom:'1px solid var(--line-2)'}}>
        <span className="w-700 t-14">{isAr?'الإشعارات':'Notifications'}</span>
        <span className="faint t-12">{events.length} {isAr?'حدث جديد':`événement${events.length>1?'s':''}`}</span>
      </div>
      <div className="scroll" style={{flex:1,overflowY:'auto'}}>
        {events.length===0 && ongoing.length===0 && <div className="muted t-14" style={{padding:'40px 20px',textAlign:'center'}}>{isAr?'كلّ شيء محدّث':'Tout est à jour'}<br/><span className="faint t-12">{isAr?'لا توجد مستجدّات منذ آخر زيارة.':'Aucune nouveauté depuis votre dernière visite.'}</span></div>}
        {events.length>0 && <>
          <div className="t-11 faint w-700" style={{padding:'10px 14px 4px',textTransform:'uppercase',letterSpacing:'.04em',background:'var(--bg)'}}>{isAr?'جديد':'Nouveau'}</div>
          {events.map(n=> <Item key={n.id} n={n}/>)}
        </>}
        {ongoing.length>0 && <>
          <div className="t-11 faint w-700" style={{padding:'12px 14px 4px',textTransform:'uppercase',letterSpacing:'.04em',background:'var(--bg)'}}>{isAr?'قيد المتابعة':'En cours'}</div>
...
function Dashboard({go}){
  ...
  const fmtNextDate = (iso)=>{
    ...
    const days = isAr
      ? ['الأحد','الإثنين','الثّلاثاء','الأربعاء','الخميس','الجمعة','السّبت']
  ...
  /* Translate the session-action label for the hero CTA. */
  const sessActLabelAr = (lbl) => {
    const m = {'Préparer la séance':'تحضير الحصّة','Saisir la séance':'تسجيل الحصّة','Voir le détail':'عرض التّفاصيل'};
    return m[lbl] || lbl;
  };
  const heroAct = sessionAction(nextSess);
  const heroActLabel = isAr ? sessActLabelAr(heroAct.label) : heroAct.label;
  return <AppShell go={go} active="dashboard" title={isAr?'مرحباً أمينة 👋':'Bonjour Amina 👋'}
    actions={<Btn variant="primary" icon={heroAct.screen==='session-plan'?'edit':'calendar'} onClick={()=>nextSess?go(heroAct.screen, heroAct.param):go('attendance')}>{heroActLabel}</Btn>}>
    <p className="muted t-15" style={{marginTop:-4,marginBottom:18}}>{isAr
      ? <>لديك <b style={{color:'var(--ink)'}}>{upcoming.length} حصّة مُبرمَجة</b> و <b style={{color:'var(--orange-600)'}}>{Nav.reportsPending().length} تقرير{Nav.reportsPending().length>1?'':''} لإرسالها</b>.</>
      : <>Vous avez <b style={{color:'var(--ink)'}}>{upcoming.length} séance{upcoming.length>1?'s':''} planifiée{upcoming.length>1?'s':''}</b> et <b style={{color:'var(--orange-600)'}}>{Nav.reportsPending().length} rapport{Nav.reportsPending().length>1?'s':''} à envoyer</b>.</>}</p>
    ...
    {nextSess && <button onClick={()=>go(heroAct.screen, heroAct.param)} className="card" style={{
      ...
      <div style={{flex:1,padding:'20px 22px',color:'#fff'}}>
        <div className="row gap-8" style={{marginBottom:4,opacity:.85}}>
          ...
          <span className="t-12 w-700" style={{textTransform:'uppercase',letterSpacing:'.06em'}}>{isAr?'الحصّة القادمة':'Prochaine séance'} · {fmtNextDate(nextSess.date)}</span>
        </div>
        <div className="row gap-10 wrap" style={{alignItems:'baseline',marginBottom:6}}>
          <h2 style={{fontSize:24,color:'#fff'}}>{nextSess.plannedTopic ? (txData?txData(nextSess.plannedTopic):nextSess.plannedTopic) : (isAr?'يُحضَّر':'À préparer')}</h2>
        ...
        <div className="row gap-14 wrap" style={{fontSize:13,opacity:.92,marginTop:8}}>
          <span className="row gap-6"><Icon name="book" size={13}/>{(nextSess.plannedItems||[]).length} {isAr?'نقطة مُبرمَجة':`point${(nextSess.plannedItems||[]).length>1?'s':''} prévus`}</span>
          {nextCarry.length>0 && <span className="row gap-6"><Icon name="clock" size={13}/>{nextCarry.length} {isAr?'لاستئنافها':'à reprendre'}</span>}
          {nextTodos.length>0 && <span className="row gap-6"><Icon name="target" size={13}/>{nextTodos.length} {isAr?'مهمّة تحضير':`tâche${nextTodos.length>1?'s':''} prépa`}</span>}
        ...
        {lastSess && <div className="row gap-6 wrap" style={{fontSize:12,marginTop:10,paddingTop:10,borderTop:'1px solid rgba(255,255,255,.18)',opacity:.85}}>
          ...
          <span><b>{isAr?'الحصّة السّابقة':'Dernière séance'}</b> · {lastSess.plannedTopic?(txData?txData(lastSess.plannedTopic):lastSess.plannedTopic):'—'}{lastSess.date?` · ${fmtDateLoc?fmtDateLoc((new Date(lastSess.date)).toLocaleDateString('fr-FR',{day:'2-digit',month:'short'})):(new Date(lastSess.date)).toLocaleDateString('fr-FR',{day:'2-digit',month:'short'})}`:''}</span>
        </div>}
      </div>
      <div style={{display:'flex',alignItems:'center',justifyContent:isMobile?'center':'flex-start',padding:isMobile?'14px':'0 24px',background:'rgba(255,255,255,.1)',color:'#fff',fontWeight:700,fontSize:14,gap:8,whiteSpace:'nowrap'}}>
        {heroActLabel} <Icon name="arrow" size={16}/>
      </div>
    </button>}
    ...
    <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'repeat(3,1fr)',gap:14,marginBottom:14}}>
      {[
        {ic:'user',t:(isAr?'إضافة تلميذ':'Ajouter un élève'),s:(isAr?'في أحد أفواجك':'Dans un de vos groupes'),id:'add-student',tn:'blue'},
        {ic:'file',t:(isAr?'توليد تقرير':'Générer un rapport'),s:(isAr?'انطلاقاً من الملاحظات':'À partir des remarques'),id:'report-gen',tn:'green'},
        {ic:'target',t:(isAr?'تلاميذ للمتابعة':'Élèves à suivre'),s:(isAr?`${Nav.studentsAll().filter(s=>s.status==='pending'||s.att<85).length} للمراجعة`:`${Nav.studentsAll().filter(s=>s.status==='pending'||s.att<85).length} à revoir`),id:'__suivi',tn:'orange'},
      ].map(a=>{
    ...
    <div id="suivi" className="card pad-20 col gap-14" style={{marginBottom:20,scrollMarginTop:16}}>
      <div className="row between">
        <h3 className="row gap-8" style={{fontSize:17}}><Icon name="target" size={18} style={{color:'var(--orange-600)'}}/>{isAr?'تلاميذ يحتاجون إلى متابعة':'Élèves nécessitant un suivi'}</h3>
        <Btn variant="soft" size="sm" iconR="arrow" onClick={()=>go('students')}>{isAr?'كلّ التّلاميذ':'Tous les élèves'}</Btn>
      </div>
      <div className="col">
        {Nav.studentsAll().filter(s=>s.status==='pending'||s.att<85).map((s,i,arr)=>{
          ...
          return <div key={s.id} className="row between wrap" style={{gap:12,padding:'13px 0',borderTop:i>0?'1px solid var(--line-2)':'none',cursor:'pointer'}} onClick={()=>go('student-profile', s.id)}>
            <div className="row gap-12" style={{minWidth:200}}>
              ...
              <div className="col" style={{gap:1}}><span className="w-600 t-14">{s.name}</span><span className="faint t-12">{(Nav.groupById(s.groupId)||{}).name?.split('—')[0]?.trim()||'—'} · {isAr?txData(s.parent):s.parent}</span></div>
            </div>
            <div className="row gap-18 wrap" style={{alignItems:'center'}}>
              <div className="col" style={{gap:3,minWidth:96}}>
                <span className="faint t-11 w-700" style={{textTransform:'uppercase',letterSpacing:'.04em'}}>{isAr?'الحضور':'Présence'}</span>
              ...
              {s.status==='pending'
                ? <span className="badge badge-pending"><Icon name="file" size={13}/>{isAr?'تقرير للإرسال':'Rapport à envoyer'}</span>
                : <span className="badge badge-new"><Icon name="eye" size={13}/>{isAr?'للمراقبة':'À surveiller'}</span>}
              <Btn variant="ghost" size="sm" iconR="chevron" onClick={(e)=>{e.stopPropagation();go('student-profile', s.id);}}>{isAr?'عرض التلميذ':'Voir l’élève'}</Btn>
            </div>
          </div>;
        })}
      </div>
    ...
    <div className="row between" style={{marginBottom:10}}>
      <h3 style={{fontSize:16}}>{isAr?'أفواجي':'Mes groupes'}</h3>
      <Btn variant="ghost" size="sm" iconR="arrow" onClick={()=>go('groups')}>{isAr?'عرض الكل':'Tout voir'}</Btn>
    </div>
      ...
      {Nav.groupsAll().map(g=>{
        ...
        return <button key={g.id} onClick={()=>go('group-detail', g.id)} className="card" style={{textAlign:'left',padding:'16px 18px',border:'1px solid var(--line)',cursor:'pointer'}}>
          <div className="row between wrap" style={{marginBottom:12,gap:8,rowGap:6}}>
            ...
            {g.places>0?<span className="chip chip-green" style={{fontSize:10.5,padding:'2px 8px'}}>{g.places} {isAr?'مكان':`place${g.places>1?'s':''}`}</span>:<span className="chip chip-weak" style={{fontSize:10.5,padding:'2px 8px'}}>{isAr?'كامل':'Complet'}</span>}
          </div>
          <SeatRow cap={g.cap} students={g.students}/>
          <div className="row gap-8 wrap" style={{marginTop:12,alignItems:'baseline',rowGap:4}}>
            <span className="t-12 muted w-600">{g.students}/{g.cap} {isAr?'تلاميذ':'élèves'}</span>
            <span className="faint t-11">·</span>
            <span className="t-12 muted w-600">{next?(isAr?`القادمة ${fmtDateLoc?fmtDateLoc(next.date):(new Date(next.date)).toLocaleDateString('fr-FR',{day:'2-digit',month:'short'})}`:`Prochaine ${(new Date(next.date)).toLocaleDateString('fr-FR',{day:'2-digit',month:'short'})}`):(isAr?'لا توجد حصة':'Pas de séance')}</span>
          </div>
        </button>;
      })}
    </div>
    ...
    <div className="card-flat" style={{padding:'10px 14px',borderRadius:10,background:'var(--bg)',border:'1px solid var(--line-2)',marginBottom:8}}>
      <span className="row gap-14 wrap t-13 muted">
        <span className="row gap-6"><Icon name="users" size={13}/><b className="tnum" style={{color:'var(--ink)'}}>{Nav.groupsAll().length}</b> {isAr?'أفواج':'groupes'}</span>
        <span className="row gap-6"><Icon name="user" size={13}/><b className="tnum" style={{color:'var(--ink)'}}>{Nav.studentsAll().length}</b> {isAr?'تلاميذ':'élèves'}</span>
        <span className="row gap-6"><Icon name="calendar" size={13}/><b className="tnum" style={{color:'var(--ink)'}}>{Nav.sessionsAll().filter(s=>s.status==='planned').length}</b> {isAr?'حصص قادمة':'séances à venir'}</span>
        <span className="row gap-6"><Icon name="file" size={13} style={{color:Nav.reportsPending().length>0?'var(--orange-600)':'var(--faint)'}}/><b className="tnum" style={{color:Nav.reportsPending().length>0?'var(--orange-600)':'var(--ink)'}}>{Nav.reportsPending().length}</b> {isAr?'تقارير للإرسال':'rapports à envoyer'}</span>
      </span>
    </div>
  </AppShell>;
}
...
function Groups({go}){
  ...
  const list = segment==='with-places' ? all.filter(g=>g.places>0)
             ...
             : all;
  return <AppShell go={go} active="groups" title={isAr?'الأفواج':'Groupes'}
    crumbs={[{t:isAr?'الفضاء':'Espace',go:()=>go('dashboard')},{t:isAr?'الأفواج':'Groupes'}]}
    actions={<Btn variant="primary" icon="plus" onClick={()=>go('create-group')}>{isAr?'إضافة فوج':'Ajouter un groupe'}</Btn>}>
    <div className="row gap-12" style={{marginBottom:18}}>
      <div className="seg">
        <button className={segment==='all'?'on':''} onClick={()=>setSegment('all')}>{isAr?'الكلّ':'Tous'} ({all.length})</button>
        <button className={segment==='with-places'?'on':''} onClick={()=>setSegment('with-places')}>{isAr?'بأماكن':'Avec places'} ({all.filter(g=>g.places>0).length})</button>
        <button className={segment==='full'?'on':''} onClick={()=>setSegment('full')}>{isAr?'كاملة':'Complets'} ({all.filter(g=>g.places===0).length})</button>
      </div>
      <span className="muted t-14 w-600" style={{marginLeft:'auto'}}>{list.length} {isAr?'فوج':'groupe'}{!isAr&&list.length>1?'s':''} · {Nav.studentsAll().length} {isAr?'تلميذاً':'élèves'}</span>
    </div>
    <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'1fr 1fr',gap:18}}>
        ...
        <div key={g.id} className="card pad-20 col gap-14">
          <div className="row between">
            <div className="col gap-6">
              ...
              <div className="row gap-7 wrap"><span className="chip chip-blue">{txData?txData(g.subject):g.subject}</span><span className="chip chip-gray">{g.level}</span></div>
            </div>
          ...
          <div className="card-flat" style={{padding:12,borderRadius:11,background:'var(--bg)',border:'none'}}>
            <div className="row between" style={{marginBottom:8}}><span className="t-13 w-600 muted">{isAr?'سعة الفوج':'Capacité du groupe'}</span><span className="t-13 w-700">{g.students} / {g.cap}</span></div>
            <SeatRow cap={g.cap} students={g.students}/>
            <div className="row between" style={{marginTop:10}}>
              <span className="row gap-6 t-12 muted w-600"><Icon name="book" size={13}/>{isAr?'آخر حصّة:':'Dernière séance :'} {txData?txData(g.last):g.last}</span>
              {g.reports>0?<span className="badge badge-pending"><Icon name="file" size={13}/>{g.reports} {isAr?'في الانتظار':'en attente'}</span>:<span className="badge badge-verified"><Icon name="check" size={13}/>{isAr?'محدَّث':'À jour'}</span>}
            </div>
          </div>
          <div className="row gap-10 wrap" style={{rowGap:10}}>
            <Btn variant="ghost" size="sm" iconR="arrow" onClick={()=>go('group-detail', g.id)}>{isAr?'فتح الفوج':'Ouvrir le groupe'}</Btn>
            {(()=>{const next=Nav.nextPlannedSession(g.id); const a=sessionAction(next);
              const actLab = isAr ? ({'Préparer la séance':'تحضير الحصّة','Saisir la séance':'تسجيل الحصّة','Voir le détail':'عرض التّفاصيل','Nouvelle séance':'حصّة جديدة'}[a.label]||a.label) : a.label;
              return <Btn variant="ghost" size="sm" icon={a.screen==='session-plan'?'edit':'calendar'} onClick={()=>go(a.screen, a.param)}>{actLab}</Btn>;})()}
            <Btn variant="soft" size="sm" icon="file" onClick={()=>go('reports-batch', g.id)}>{isAr?'التّقارير':'Rapports'}</Btn>
          </div>
        </div>)}
      {/* add card */}
      <button className="card" onClick={()=>go('create-group')} style={{padding:20,borderRadius:16,border:'1.5px dashed var(--line)',background:'transparent',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:10,minHeight:160,cursor:'pointer',color:'var(--muted)'}}>
        ...
        <span className="w-600 t-15">{isAr?'إنشاء فوج جديد':'Créer un nouveau groupe'}</span>
      </button>
    </div>
  </AppShell>;
}
